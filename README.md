...
- 「もっと読み込む」ボタンで追加50件ずつ取得可能
- WebSocketで未承認→承認のリアルタイム更新（ピンクマーカーで「新着」表示し、承認後に震度色へ更新）
...
### 4. 音声通知
- ヘッダーの「🔔 通知ON/🔕 通知OFF」ボタンで切り替え
- P2P地震速報で新着を検知したとき、震度に応じた回数・周波数のビープ音を鳴らす
- Web Audio API を使用（ユーザー操作後に有効化）
...
5. ×ボタンで通知を閉じる
6. リスト末尾の「もっと読み込む」ボタン → 過去の地震を追加取得
7. ヘッダーの「🔕 通知OFF」ボタンをクリック → 音声通知を有効化
